...
import { groundSequenceIds, type GroundSequenceId } from '@/features/ground/steps';
...
type Props = {
  selectedId: GroundSequenceId;
  onSelect: (id: GroundSequenceId) => void;
};

const ITEM_MIN_WIDTH = 108;
const ITEM_GAP = spacing.sm;

export function GroundSequencePicker({ selectedId, onSelect }: Props) {
  const { theme } = useTheme();
  const haptics = useHaptics();
  const scrollRef = useRef<ScrollView>(null);
  const itemX = useRef<Partial<Record<GroundSequenceId, number>>>({});

  const scrollToId = useCallback((id: GroundSequenceId, animated: boolean) => {
  ...
  return (
    <HorizontalTextCarouselFrame>
      <ScrollView
        ...
        accessibilityLabel={t('ground.menu')}
        showsHorizontalScrollIndicator={false}
        contentContainerStyle={styles.content}
        style={styles.scroll}
      >
        {groundSequenceIds.map((id, index) => {
        const selected = id === selectedId;
        const label = t(`ground.sequences.${id}`);
        return (
          <AccessiblePressable
            key={id}
            accessibilityRole="radio"
            accessibilityLabel={label}
            accessibilityHint={t('ground.sequenceHint')}
            accessibilityState={{ selected }}
            onLayout={(event) => {
              itemX.current[id] = event.nativeEvent.layout.x;
              if (id === selectedId) {
                scrollToId(id, false);
              }
            }}
